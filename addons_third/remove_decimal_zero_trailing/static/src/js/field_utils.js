...
function formatFloat(value, field, options) {
    ...
    var formatted = _.str.sprintf('%.' + precision + 'f', value || 0).replace(".",l10n.decimal_point).split(l10n.decimal_point);
    formatted[0] = utils.insert_thousand_seps(formatted[0]);
    // return formatted.join(l10n.decimal_point).replace(/0+$/g, "").replace(/[.]$/,"");
    var arrNum = formatted.join(l10n.decimal_point).split(l10n.decimal_point)
    var regx = "/["+l10n.decimal_point+"]$/"
    if(arrNum.length > 1 && arrNum[1].replace(/0+$/g, "").replace(regx,"") !== "") {
        return arrNum[0] + l10n.decimal_point + arrNum[1].replace(/0+$/g, "").replace(regx, "");
    } else {
        return arrNum[0]
    }
}
