...
class KsDashboardNinjaBoard(models.Model):
    ...
    ks_set_interval = fields.Selection(
        [('15000', '15 Seconds'), ('30000', '30 Seconds'), ('45000', '45 Seconds'), ('60000', '1 minute'),
         ('120000', '2 minute'), ('300000', '5 minute'), ('600000', '10 minute')], string="Update Interval")
    ...
    def ks_fetch_item_data(self, rec):
        ...
        item = {
            ...
            'ks_chart_data': ks_chart_data,
            'ks_list_view_data': ks_list_view_data,
    ...
    # [04-07-2022] ThienDV add method import default dashboard from path files
    @api.model
    def ks_import_dashboard_from_paths(self, str_paths):
        if not isinstance(str_paths, str):
            return
        str_paths = str_paths.replace('\n', ' ').replace('\r', '').replace(' ', '')
        paths = str_paths.split(',')
        for path in paths:
            current_file = __file__
            parent = current_file.split('ks_dashboard_ninja')
            if len(parent) <= 1:
                return
            f_path = parent[0] + path
            file = open(f_path, "r")
            data_json = file.read()
            self.ks_import_dashboard(data_json)
            print(f_path)
        ...
        for data in ks_dashboard_data:
            ...
            if data['ks_item_data']:
                ...
                for item in data['ks_item_data']:
                    ...
                    item['ks_item_start_date_2'] = datetime.datetime.strptime(item['ks_item_start_date_2'],
                                                                              '%b %d, %Y') if \
                        item[
                    ...
                    # [01-07-2022] Updated: remove ks_id in dict "item"
                    ks_id = None
                    if item.get('ks_id'):
                        ks_id = item.get('ks_id')
                        item.pop('ks_id')
                    ...
                    if data['ks_gridstack_config'] and str(ks_id) in ks_gridstack_config:
